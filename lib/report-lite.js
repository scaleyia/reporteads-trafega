...
export function montaCfgCliente(cliente, accounts) {
  ...
  // Gráfico: SEMPRE as 6 últimas semanas (segundas). Soma, para cada semana,
  // só os pontos das contas que têm exatamente aquele rótulo — descarta dado
  // antigo (mensal) que não bate, evitando o gráfico "Frankenstein".
  const ids = new Set((cliente.contas || []).map((x) => x.id || x));
  const contas = accounts.filter((a) => ids.has(a.id));
  const labels = [];
  for (let i = 5; i >= 0; i--) { const m = new Date(prevMon); m.setDate(prevMon.getDate() - i * 7); labels.push(dd(m)); }
  const valores = labels.map((lb) => {
    let soma = 0;
    for (const a of contas) {
      const s = a.serie || { labels: [], valores: [] };
      const idx = s.labels.indexOf(lb);
      if (idx >= 0) soma += Number(s.valores[idx]) || 0;
    }
