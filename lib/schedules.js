...
/* --------------------------- Disparo ----------------------------------- */
function activeClientNames() {
  try {
    const list = JSON.parse(fs.readFileSync(CLIENTS_FILE, 'utf-8'));
    return list.map((c) => c.nome);
  } catch { return []; }
}

/* Hook de disparo. Quando o envio no WhatsApp + dados (Google/Meta) existirem,
   é aqui que cada relatório é gerado e enviado. Por ora, registra como pendente. */
async function dispatchSchedule(s, clientes) {
  // TODO(integração): gerar o PDF (lib/report + dados reais) e enviar via
  // Evolution para o WhatsApp de cada cliente. Retornar status por cliente.
  return { status: 'pendente', motivo: 'Aguardando integração de envio/dados' };
}
...
export async function runDue(now = new Date(), opts = {}) {
  ...
  for (const s of list) {
    ...
    const clientes = s.clientes === 'todos' ? activeClientNames() : s.clientes;
