...
import { buildResumoHtml, montaResumoCliente } from './report-lite.js';
...
async function dispatchSchedule(s, clientesNomes) {
  ...
  for (const cliente of alvo) {
    try {
      ...
      const resumo = montaResumoCliente(cliente, accounts);
      if (resumo.semContas) throw new Error('sem contas vinculadas');
      const pdf = await htmlToPdf(buildResumoHtml(resumo));
      const base64 = Buffer.from(pdf).toString('base64');
      const legenda = 'Olá! Segue o relatório de ' + resumo.periodo + ' — ' + cliente.nome + '. 📊';
